...
def calc_eps_growth(fundamentals):
    previous = fundamentals[1]['earningsPerBasicShare']
    current = fundamentals[0]['earningsPerBasicShare']
    return (current-previous)/(previous)

def get_check_for_buys():
    to_buy = []
    for ticker in S_AND_P_500:
        fundamentals = pw.get_fundamentals(ticker)['results']
        try:
            if fundamentals[0]['debtToEquityRatio'] <= 1.5 and fundamentals[0]['priceToBookValue'] < 1:
                if calc_eps_growth(fundamentals) > 0:
                    ddm = calc_dividend_discount(fundamentals[0])
                    dcf = calc_discounted_cash_flow(fundamentals[0])
                    rim = calc_residual_income_model(fundamentals[0])
                    margin_of_safety = (ddm+dcf+rim)/3
